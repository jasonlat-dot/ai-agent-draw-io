...
```
ai-agent-draw-io/
├── ai-agent-draw-io-api          # API 层
├── ai-agent-draw-io-app          # 应用层（配置文件、测试）
├── ai-agent-draw-io-domain       # 领域层（Agent 核心逻辑）
├── ai-agent-draw-io-infrastructure # 基础设施层
├── ai-agent-draw-io-trigger      # 触发层（HTTP 接口）
├── ai-agent-draw-io-types        # 类型定义
